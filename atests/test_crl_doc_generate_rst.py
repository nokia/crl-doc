...
PIP_INDEX_URL = 'https://pypi.dynamic.nsn-net.net/crl/prod'
...
class DocRunnerBase(object):

    spec_arg = ''
    requirements = ['configobj',
                    'crl.examplelib==1.0.6',
...
def assert_dirs_are_same_modulo_generated(adir, bdir):
    assert _path_lines_list(adir) == _path_lines_list(bdir)
...
def assert_generators_are_same(agen, bgen):
    for a in agen:
        assert [a for a in agen] == [b for b in bgen]
