...
from ute_doc.robotws_util import add_toc_tree  # pylint: disable=import-error
from ute_doc.util import create_dir
...
class CRLDocBuilder(object):
    # pylint: disable=too-many-instance-attributes
    """ CRLDocBuilder generates Sphinx documentation source from Robot
    Framework test libraries and UTE common API as default. Document type
    ...
    **Example usage**:
    ...
    def __init__(self, args):
        self.static = pkg_resources.resource_filename('ute_doc', 'resource')
        ...
        self.rst_ute_common_api = os.path.join(self.rst_root, 'ute_common_api')
    ...
    def _generate_robotdocsconfs_docs(self):
        for r in self._args.robotdocsconfs:
            self._generate_docs_for_robotdocs(r)
    ...
    def _generate_builtin_documentation(self):
        for builtin_lib in os.listdir(get_robot_builtin_path()):
            if (builtin_lib.endswith('.py') and
                ...
                generate_builtin_doc(builtin_lib, UTE_TEMPLATE_DIR,
                                     self.xml_root, self.rst_root)

    def _generate_api_documentation(self):
        self._generate_ute_api_documentation()
        self._generate_api_documentation_for_crl_namespaces()

    def _generate_ute_api_documentation(self):
        self._generate_api_documentation_with_filter(
            dist_filter=lambda dist: dist.startswith('ute_common'),
            out=self.rst_ute_common_api,
            dist_translate=lambda dist: str(dist.replace('-', '_')))
    ...
    def _add_toc_tree(self):
        add_toc_tree(self.rst_root, UTE_TOC_TEMPLATE)
