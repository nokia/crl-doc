...
from ute_doc.rst import RstTableCreator
from ute_doc.util import convert_xml_to_rst, create_dir
from ute_doc.robotws_util import create_dir as wscreate_dir
...
class RobotXmlRstFile(object):
    ...
    @property
    def author(self):
        return self.metadata.get('Author', '').decode('utf-8')
    ...
    def robot_xml_rst_files(self):
        for r in self._robotdocsentrypoints:
            for library, args in r.robotdocsconf.items():
                with error_handling(
                    'Failed to generate Robot documentation'
                    ' for library {}'.format(
                        library)):
